		...
		>
			<template v-for="(menu,index) in routeArray"><!-- 把for循环从el-menu中拿出来放入template里，不然unique-opened不会生效 -->
				<el-submenu v-if="isSubmenu(menu)" :index="menu.path" :key="index">
					<template slot="title">
					    <i :class="menu.icon"></i>
					    <span slot="title">{{menuName(menu)}}</span>
					</template>
					<el-menu-item v-if="!item.hidden" v-for="(item,cIndex) in menu.children" :index="menu.path+'/'+item.path" :key="cIndex">
						{{item.name}}
					</el-menu-item>
				</el-submenu>
				<el-tooltip v-else-if="!menu.hidden" class="item" effect="dark" :content="menuName(menu)" placement="right" :disabled="!isCollapse">
...
<script>
	...
	export default{
		...
		methods:{
			menuName(menu){
				if(menu.name){//路由中，根路由设置了name直接返回
					return menu.name;
				}else{
					if(menu.children&&menu.children.length>0){//路由中，根路由未设置name则使用第一个children的name
						return menu.children[0].name;
					}else{
						throw "we need a menu name";
					}
				}
			},
			isSubmenu(menu){
